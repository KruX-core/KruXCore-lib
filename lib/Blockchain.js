...
class Blockchain { /**
    ...
    airdropCoins() {
        ...
        for (const address  of this._registeredAddresses) {
            this._pendingTxns.push(new Transaction(Date.now(), 'Airdrop', address, coinsPerWallet));
        }
    ...
    createGenesisBlock() {
        ...
        let genesisBlock = new Block('1556735351', [ txn ], '0', this._options.verbose);
    ...
    mineCurrentBlock(minerAddress, minedByAirdrop = false) {
        ...
        for (const txn of this._pendingTxns) {
            ...
            case 'BlockMinting':
                ...
                break;
            case 'Airdrop':
                xa.custom('Blockchain', `Transaction validated: Airdrop (${
                    txn.amount
                } coins) for ${
                    txn.recipient.trunc(32)
                }`, {
        ...
        this._pendingTxns = [ new Transaction(Date.now(), 'BlockMinting', minerAddress, this._blockReward) ];
        // this._pendingTxns = [];
    }
    ...
    addTransaction(txn) {
        ...
        if (this._options.verbose) {
            xa.custom('Blockchain', `Transaction ${
                this._pendingTxns.length
            } added`, {
                titleColor: '#69E45E',
                backgroundColor: '#322222'
            });
