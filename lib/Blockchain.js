...
class Blockchain { /**
    ...
    createGenesisBlock() {
        ...
        let genesisBlock = new Block(this._options.genesisTimestamp, [ txn ], '0');
        if (this._options.verbose) {
            xa.custom('Blockchain', 'Created genesis block', { titleColor: '#45D339', backgroundColor: '#453232' });
        }
    ...
    mineCurrentBlock(minerAddress, minedByAirdrop = false) {
        ...
        xa.custom('Blockchain', `Miner will recieve ${minerReward} coins as reward`, { titleColor: '#45D339', backgroundColor: '#453232' });
     ...
     */
    validateTx(txn) {
        const balance = this.getBalanceForAddress(txn.sender);

        if (txn.fee < this._options.txFee) {
            xa.custom('Blockchain', 'Fee too low', { titleColor: '#E42626', backgroundColor: '#322222' });
            return false;
        }
    ...
    addTransaction(txn) {
        if (!txn.sender || !txn.recipient) {
            xa.custom('Blockchain', 'Transaction must include the sender\'s and the recipient\'s addresses and the fee', { titleColor: '#E42626', backgroundColor: '#322222' });
            return;
        }
        ...
        if (this._options.verbose) {
            xa.custom('Blockchain', `Transaction ${this._pendingTxns.length} added with ${txn.fee} coin fee`, { titleColor: '#69E45E', backgroundColor: '#322222' });
        }
    ...
    addWalletAddress(address) {
        if (this._registeredAddresses.indexOf(address) > -1) {
            if (this._options.verbose) {
                xa.custom('Blockchain', `${address.trunc(48)} already registered`, { titleColor: '#E42626', backgroundColor: '#322222' });
            }
        ...
        if (this._options.verbose) {
            xa.custom('Blockchain', `${address.trunc(48)} successfully registered!`, { titleColor: '#69E45E', backgroundColor: '#322222' });
        }
    }
